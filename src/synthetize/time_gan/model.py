...
from src.synthetize.time_gan import gan
...
def make_net(model, n_layers, hidden_units, output_units):
    ...
    model.add(Dense(units=output_units, activation="tanh", name="OUT"))
    return model
...
class TimeGAN(gan.Model):
    ...
    def train(self, data, train_steps, save_dir):
        ...
        for _ in tqdm(range(train_steps), desc="Joint networks training"):
            ...
            for _ in range(2):
                ...
                # Train the generator
                # --------------------------
                step_g_loss_u, step_g_loss_s, step_g_loss_v = self.train_generator(
                    X_, Z_
                )
